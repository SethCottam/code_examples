...
WAZE_URL = "https://na-georss.waze.com/rtserver/web/TGeoRSS?tk=ccp_partner&ccp_partner_name={PARTNER_NAME},%20Inc.&format=JSON&types=alerts&polygon="
...
class NationalWazeConnector():
	...
	def __init__(self, *args, **kwargs):
		...
		if VERBOSE:
			...
			# self.weather = 1
			# self.thing = 0
			# self.time = 12.213123123
			# self.number = 5

			# print(f'{DEBUG1}Weather:{RESET} \t{self.warning_check(self.weather)}')
			# print(f'{DEBUG1}Weather:{RESET} \t{self.success_check(self.weather)}')
			# print(f'{DEBUG1}time:{RESET} \t{self.runtime_check(self.time, self.number)}')
	...
	def get_signals(self, region, polygon):
		...
		raw_file = open(f'output/raw/{self.file_timestamp}_{region}.json', 'wb')
		...
		filtered_file = open(f'output/filtered/{self.file_timestamp}_{region}.json', "w")
